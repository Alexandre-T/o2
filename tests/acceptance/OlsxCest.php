...
{
    /**
     * Try to test the OLSX service as accountant.
     *
     * @param AcceptanceTester $you the acceptance tester
     */
    public function tryToCallTestAsAccountant(AcceptanceTester $you): void
    {
        $you->wantTo('call the test page');
        $you->login('accountant');
        $you->areOnPage('/olsx/test');
        $you->seeResponseCodeIsSuccessful();
        $you->see('le service OLSX a bien répondu');
    }

    /**
     * Try to test the OLSX service as administrator.
     *
     * @param AcceptanceTester $you the acceptance tester
     */
    public function tryToCallTestAsAdministrator(AcceptanceTester $you): void
    {
        $you->wantTo('call the test page');
    ...
    public function tryToListRegisteringUsers(AcceptanceTester $you): void
    {
        ...
        $you->see('Aucun');
        $you->see('Client simple', '.badge');
        $you->wantTo('test convert the 11111 customer as a personal customer');
        $you->areOnPage('/olsx/personal/'.$identifier);
        $you->seeResponseCodeIsSuccessful();
        $you->seeCurrentUrlEquals('/olsx/show/'.$identifier);
        $you->see('Le service OLSX nous a répondu que ce numéro de compte-client n’existait pas. Requête annulée');
        ...
        $you->see('Client simple', '.badge');
        $you->dontSee('Client OLSX', '.badge');
        $you->wantTo('test convert the personal 33333 customer as a personal customer');
        $you->areOnPage('/olsx/personal/'.$identifier);
        $you->seeResponseCodeIsSuccessful();
        $you->seeCurrentUrlEquals('/olsx/show/'.$identifier);
        $you->see('Le service OLSX nous a répondu que ce client dispose d’ores-et-déjà  d’un compte personnel');
